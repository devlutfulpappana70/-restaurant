...
class Savour_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = savour()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'savour_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'savour_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->savour_generate_file_options();
		} );
	}

	function savour_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->savour_generate_file_options();
			$this->savour_generate_file();
		}
	}

    function savour_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->savour_options_output() )
        ) );
    }

	function savour_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function savour_options_output() {
		$theme_colors                    = savour_configs('theme_colors');
        $links                           = savour_configs('link');
        $gradients                       = savour_configs('gradient');
		ob_start();

		printf('$savour_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$savour_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function savour_enqueue() {
        $css = $this->savour_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function savour_inline_css() {
        ob_start();
