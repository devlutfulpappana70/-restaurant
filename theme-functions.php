 ...
 * Google Fonts
*/
function savour_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    $subsets   = 'latin,latin-ext';

    if ( 'off' !== _x( 'on', 'Roboto font: on or off', 'savour' ) ) {
        $fonts[] = 'Roboto:ital,wght@0,400;0,500;0,600;0,700';
    }

    if ( 'off' !== _x( 'on', 'Lora font: on or off', 'savour' ) ) {
        $fonts[] = 'Lora:ital,wght@0,400;0,500;0,600;0,700;0,800;1,400;1,500;1,600;1,700;1,800';
    }

    if ( 'off' !== _x( 'on', 'Fira Sans font: on or off', 'savour' ) ) {
        $fonts[] = 'Fira Sans:ital,wght@1,400;1,700;';
    }

    if ( 'off' !== _x( 'on', 'Playfair Display SC font: on or off', 'savour' ) ) {
        $fonts[] = 'Playfair Display SC:ital,wght@1,400;1,700;';
    }
 ...
 * Get page ID by Slug
*/
function savour_get_id_by_slug($slug, $post_type){
 ...
 **/
function savour_content_by_slug($slug, $post_type){
    $content = savour_get_content_by_slug($slug, $post_type);

    $id = savour_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function savour_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
 ...
 * Custom Comment List
 */
function savour_comment_list( $comment, $args, $depth ) {
	if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 * Paginate Links
 */
function savour_ajax_paginate_links($link){
 ...
 * Image Size Crop
 */
if(!function_exists('savour_get_image_by_size')){
    function savour_get_image_by_size( $params = array() ) {
 ...
 * Search Form
 */
function savour_header_mobile_search_form() { 
    $search_mobile = savour()->get_theme_opt( 'search_mobile', false );
    $search_placeholder_mobile = savour()->get_theme_opt( 'search_placeholder_mobile' );
    if($search_mobile) : ?>
    <div class="pxl-header-mobile-search pxl-hide-xl">
        <form role="search" method="get" action="<?php echo esc_url(home_url( '/' )); ?>">
            <input type="text" placeholder="<?php if(!empty($search_placeholder_mobile)) { echo esc_attr($search_placeholder_mobile); } else { esc_attr_e('Search...', 'savour'); } ?>" name="s" class="search-field" />
...
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_year_shortcode() {
        ob_start(); ?>
            <span><?php the_date('Y'); ?></span>
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_year', 'savour_year_shortcode');
}

/* Highlight Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_text_highlight_shortcode( $atts = array() ) {
    ...
    pxl_register_shortcode('highlight', 'savour_text_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function savour_text_highlight_shortcode_editor( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_highlight', 'savour_text_highlight_shortcode_editor');
}

/* Typewriter Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_text_typewriter_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('typewriter', 'savour_text_typewriter_shortcode');
}

/* Button Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_btn_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
         'style' => 'btn-slider1',
         'icon_class' => '',
    ...
    pxl_register_shortcode('pxl_button', 'savour_btn_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function savour_slider_arrow( $atts = array() ) {
        extract(shortcode_atts(array(
         'type' => 'next',
         'style' => 'style-1',
        ), $atts));
    ...
    pxl_register_shortcode('slider_arrow', 'savour_slider_arrow');
}

/* Gallery Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_gallery_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link' => '#',
         'images_id' => '',
         'cols' => '2',
        ...
        ob_start();
        ?>
        <div id="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>" class="pxl-gallery gallery-<?php echo esc_attr($cols); ?>-columns <?php if(!empty($masonry)) { echo 'masonry-'.esc_attr($masonry); } ?>">
        <?php
        ...
        foreach ($pxl_images as $key => $img_id) :
            ...
            <div class="pxl--item">
                <div class="pxl--item-inner <?php if($key == 1 && !empty($link) && $btn_video == 'yes' ) { echo 'video-active'; } ?>">
                    <a href="<?php echo esc_url($thumbnail_url); ?>" data-elementor-lightbox-slideshow="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>"><?php echo savour_html($thumbnail); ?></a>
    ...
    pxl_register_shortcode('pxl_gallery', 'savour_gallery_shortcode');
}

/* Addd shortcode Video button */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_video_button_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link' => '',
         'text' => '',
         'class' => 'style1',
    ...
    pxl_register_shortcode('pxl_video_button', 'savour_video_button_shortcode');
}

/* Get Category Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_post_category_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'items' => '6',
         'columns' => '2',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_post_category', 'savour_post_category_shortcode');
}

/* Slider 1  */
if(function_exists( 'pxl_register_shortcode' )) {
    function savour_slider_price_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'price' => '',
         'desc' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_slider_price', 'savour_slider_price_shortcode');
}

/**
 * Custom Widget Categories - Count
 */
add_filter('wp_list_categories', 'savour_wg_category_count');
function savour_wg_category_count($output) {
 ...
 */
add_filter('get_archives_link', 'savour_wg_archive_count');
function savour_wg_archive_count($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
add_filter('wp_list_categories', 'savour_wc_cat_count_span');
function savour_wc_cat_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
function savour_get_mega_menu_builder_id(){
    $mn_id = [];
 ...
 */
function savour_get_page_popup_builder_id(){
    $pp_id = [];
...
function savour_mouse_move_animation() { 
    $mouse_move_animation = savour()->get_theme_opt('mouse_move_animation', false); 
    if($mouse_move_animation) {
        wp_enqueue_script( 'savour-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array( 'jquery' ), '1.0.0', true ); ?>  
 ...
 * Start - Cookie Policy
 */
function savour_cookie_policy() {
    $cookie_policy = savour()->get_theme_opt('cookie_policy', 'hide');
    $cookie_policy_description = savour()->get_theme_opt('cookie_policy_description');
    $cookie_policy_btntext = savour()->get_theme_opt('cookie_policy_btntext');
    $cookie_policy_link = get_permalink(savour()->get_theme_opt('cookie_policy_link')); 
 ...
 */
function savour_subscribe_popup() {
    $subscribe = savour()->get_theme_opt('subscribe', 'hide');
    $subscribe_layout = savour()->get_theme_opt('subscribe_layout');
    $popup_effect = savour()->get_theme_opt('popup_effect', 'fade');
    $args = [
 ...
 */
add_action( 'show_user_profile', 'savour_user_fields' );
add_action( 'edit_user_profile', 'savour_user_fields' );
function savour_user_fields($user){
    ...
    ?>
    <h3><?php esc_html_e('Theme Custom', 'savour'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="user_name"><?php esc_html_e('Author Name', 'savour'); ?></label></th>
            <td>
        ...
        <tr>
            <th><label for="user_position"><?php esc_html_e('Author Position', 'savour'); ?></label></th>
            <td>
...
add_action( 'personal_options_update', 'savour_save_user_custom_fields' );
add_action( 'edit_user_profile_update', 'savour_save_user_custom_fields' );
function savour_save_user_custom_fields( $user_id )
{
...
function savour_get_user_position() {
...
/* Author Social */
function savour_get_user_social() {
